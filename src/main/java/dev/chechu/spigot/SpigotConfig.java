...
public abstract class SpigotConfig implements Configuration {
    ...
    public SpigotConfig(JavaPlugin plugin) {
        ...
        createFiles();
        if(fixVersions()) logger.info(Message.get("config_fix"));
    }

    @Override
    public void createFiles() {
        if (!plugin.getDataFolder().exists()) if(!plugin.getDataFolder().mkdir()) {
            logger.severe(Message.get("config_folder_creation_error"));
        }
        plugin.saveDefaultConfig();
        File configFile = new File(plugin.getDataFolder() + "config.yml");
        if (!configFile.exists()) logger.severe(Message.get("config_file_creation_error"));
    }
